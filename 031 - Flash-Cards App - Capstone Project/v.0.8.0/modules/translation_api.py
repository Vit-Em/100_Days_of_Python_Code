...
import time as tm
import random as rd
from .flashcard_config import APIConfig
...
def translate_word(word: str, source_language: str, target_language: str):
    """Übersetzt ein einzelnes Wort mithilfe von GoogleTranslator."""
    try:
        translated_word = GoogleTranslator(
            source=source_language, target=target_language
        ).translate(word)
        return [translated_word]  # Return as list to match expected format
    except Exception as e:
        print(f"Fehler beim Übersetzen von '{word}': {e}")
        return None

def save_word_pairs(word_pairs, filename=APIConfig.WORD_PAIRS_FILENAME):
...
def main():
    # Spracheinstellungen
    source_language = APIConfig.DEFAULT_SOURCE_LANGUAGE  # Quellsprache
    target_language = APIConfig.DEFAULT_TARGET_LANGUAGE  # Zielsprache
    word_count = APIConfig.WORD_COUNT  # Anzahl der Wörter, die gesammelt werden sollen
