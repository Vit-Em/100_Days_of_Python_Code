...
from .flashcard_config import resource_path
...
def load_data():
    ...
    try:
        # CSV-Datei einlesen mit Fehlerbehandlung für problematische Zeilen
        file_path = resource_path("data/Words_deu-rus_v1.csv")
...
def load_api_data(fetch_api_word_pairs_func, source_language, target_language, window, update_data_func):
    """Führt das Laden von API-Daten im Hintergrund aus."""
    import threading

    threading.Thread(target=_load_api_data_threaded,
                    args=(fetch_api_word_pairs_func, source_language, target_language, window, update_data_func),
                    daemon=True).start()


def _load_api_data_threaded(fetch_api_word_pairs_func, source_language, target_language, window, update_data_func):
    """Lädt API-Daten und aktualisiert die Daten."""
    try:
        if fetch_api_word_pairs_func:
            ...
            # API-Daten abrufen
            for word in words:
                word_pairs = fetch_api_word_pairs_func(
            ...
            window.after(0, update_data_func, api_data)
    except Exception as e:
